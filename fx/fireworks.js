...
class Fireworks extends OverFxScene {
  constructor (config={}) {
    ...
    this.load.image('fsparks', `${this.config.image_path}/white.png`);
		...
		this.load.audio('f1', [`${this.config.audio_path}/firework4.mp3`]);
		this.load.audio('f2', [`${this.config.audio_path}/firework2.mp3`]);
		this.load.audio('f3', [`${this.config.audio_path}/firework3.mp3`]);
	}
	...
	pop2(t) {
      ...
      gravityY: 600,
		...
		this.audio_play_detune('f2',-1500,1500)
	}
	...
	pop3(t) {
      ...
      gravityY: 0,
		...
		this.audio_play_detune('f3',-1500,1500)
	}
    ...
    var p5 = this.emitters[this.emitters.length - 1]
		this.audio_play_detune('f2',-1500,1500)
    setOverTimeout(() => { p5.gravityY = 400 },800);
    if (recurse == true) { setOverTimeout(this.pop5.bind(this,{x: t.x, y: t.y},false),400); }
	}
}
