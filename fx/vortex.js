class Vortex extends OverFxScene {
  constructor(config={}) {
    super(config);
  ...
  fx_create() {
    var baseShader = new Phaser.Display.BaseShader('BufferShader1', fragmentShader7);
    var shader = this.add.shader(baseShader, this.w/2, this.h/2, this.w, this.h);
    setOverTimeout(()=> { this.kill_scene(); }, (this.hold + (this.fade*4)))
    shader.setRenderToTexture('shaderTexture' + this.key)
    var img = this.add.image(this.w/2, this.h/2, 'shaderTexture' + this.key)
    img.alpha = 0;
    this.tweens.add({
      targets: img,
      duration: this.fade,
